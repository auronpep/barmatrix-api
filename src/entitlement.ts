...
function checkoutFullName(session: Stripe.Checkout.Session): string | null {
  const customFirst = checkoutCustomTextValue(session, "first_name");
  const customLast = checkoutCustomTextValue(session, "last_name");
  const customFullName = [customFirst, customLast].filter(Boolean).join(" ");

  return clean(customFullName) ?? clean(session.customer_details?.name);
}
